...
### 5.1. Работа с Kafka (Создание топиков вручную)

Хотя многие системы могут автоматически создавать топики при первом обращении, иногда может потребоваться создать их вручную, например, для настройки или отладки.

Для создания топиков Kafka, которые используются в этом проекте, можно использовать утилиту `kafka-topics.sh`, поставляемую с Kafka. Если вы запускаете Kafka через Docker Compose, как описано в этом README (сервис `kafka`), вам нужно будет выполнить команды внутри контейнера `kafka`.
...
1.  **Войдите в контейнер Kafka:**
    ...
    ```
    Замените `kafka` на имя вашего контейнера Kafka, если оно отличается (можно проверить командой `docker ps`).

2.  **Создайте топики:**
    После входа в командную строку контейнера Kafka, выполните следующие команды:

    *   **Для топика `auth_events`** (используется Сервером Аутентификации для логирования событий входа, регистрации и ошибок аутентификации):
        ```bash
        kafka-topics --create --topic auth_events --bootstrap-server localhost:9092 --replication-factor 1 --partitions 1
        ```

    *   **Для топика `tank_coordinates_history`** (используется Игровым Сервером для логирования истории изменения координат танков):
        ```bash
        kafka-topics --create --topic tank_coordinates_history --bootstrap-server localhost:9092 --replication-factor 1 --partitions 1
        ```

    **Примечание по адресу брокера:**
    *   Внутри контейнера `kafka` используется адрес `localhost:9092` для `--bootstrap-server`, так как это внутренний порт Kafka в Docker-сети.
    *   Если вы пытаетесь получить доступ к Kafka с вашей хост-машины (например, с помощью другого инструмента), вам, скорее всего, потребуется использовать порт, проброшенный в `docker-compose.yml`, например, `localhost:29092`.

    Параметры `--replication-factor 1 --partitions 1` подходят для локальной разработки. В продакшн-среде эти значения обычно больше для обеспечения отказоустойчивости и производительности.

После выполнения этих команд топики будут созданы и готовы к использованию серверами.
...
### Запуск тестов и конфигурация окружения

#### Настройка PYTHONPATH
Для корректного импорта модулей проекта при запуске тестов из корневой директории может потребоваться указать `PYTHONPATH`. Это гарантирует, что `pytest` сможет найти ваши пакеты (`auth_server`, `game_server`, `core`).

Пример команды:
```bash
PYTHONPATH=. pytest
# или, если PYTHONPATH уже установлен:
# PYTHONPATH=$PYTHONPATH:. pytest
```

#### Настройка `pytest-asyncio` для асинхронных тестов
Для выполнения асинхронных тестов (написанных с использованием `async def`) используется плагин `pytest-asyncio`. Он включен в файл `requirements.txt` и должен установиться вместе с другими зависимостями для разработки. `pytest-asyncio` автоматически обнаруживает и запускает асинхронные тесты.

#### Использование переменной окружения `USE_MOCKS`
Переменная окружения `USE_MOCKS` управляет поведением клиентов внешних сервисов (таких как Redis, Kafka, RabbitMQ) во время тестов:

*   **`USE_MOCKS="true"`**: При установке этого значения клиенты будут использовать мок-объекты (заглушки) вместо реальных подключений к сервисам. Это полезно для юнит-тестирования в изоляции, когда внешние сервисы недоступны или их вызов нежелателен.
*   **`USE_MOCKS="false"`** (или если переменная **не установлена**): Клиенты будут пытаться подключиться к реальным экземплярам сервисов, используя конфигурацию из переменных окружения (например, `REDIS_HOST`, `KAFKA_BOOTSTRAP_SERVERS`). Этот режим подходит для интеграционных тестов или при тестировании взаимодействия с реальными зависимостями.

Пример запуска тестов с явным указанием режима (например, для `test_redis_client.py`, чтобы он попытался использовать реальный Redis):
```bash
USE_MOCKS="false" PYTHONPATH=. pytest tests/unit/test_redis_client.py
```
По умолчанию, если `USE_MOCKS` не задана, тесты, проверяющие реальное взаимодействие (например, `test_real_client_ping_connection_error` в `test_redis_client.py`), попытаются установить соединение, что может привести к ошибке, если соответствующий сервис не запущен и не доступен. Большинство юнит-тестов в проекте настроены на использование моков автоматически или через установку `USE_MOCKS="true"` в самих тестовых файлах для специфичных сценариев.

### Примеры команд для запуска тестов
Вот несколько общих примеров команд для запуска тестов с учетом `PYTHONPATH`:

*   **Запуск всех тестов** в директории `tests/`:
    ```bash
    PYTHONPATH=. pytest
    ```
*   **Запуск всех юнит-тестов** (предполагая, что они в `tests/unit/`):
    ```bash
    PYTHONPATH=. pytest tests/unit/
    ```
*   **Запуск тестов в конкретном файле**:
    ```bash
    PYTHONPATH=. pytest tests/unit/test_auth_service.py
    ```
*   **Запуск тестов с подробным выводом (`verbose`)**:
    ```bash
    PYTHONPATH=. pytest -v
    ```
*   **Запуск тестов с выводом print() сообщений (`-s`)**:
    ```bash
