...
async def authenticate_user(username, password):
    logger.warning("Deprecated: Called global authenticate_user. Use UserService instance instead.")
    service = UserService()
...
async def register_user(username, password): # Оригинальный register_user принимал сырой пароль
    logger.warning("Deprecated: Called global register_user. Use UserService instance and ensure password is hashed for create_user.")
    ...
    await asyncio.sleep(0.01)
    if username in MOCK_USERS_DB:
        return False, "User with this name already exists (deprecated global call)."
    MOCK_USERS_DB[username] = password # Сохраняем как есть, имитируя старое поведение
    return True, f"User {username} successfully registered (deprecated global call)."

async def create_user(username, password_hash): # Добавим и create_user для полноты
    logger.warning("Deprecated: Called global create_user. Use UserService instance instead.")
    service = UserService()
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        logger.info(f"Auth Test 1: {auth_success}, {auth_msg}")
        auth_success, auth_msg = await user_service_instance.authenticate_user("player1", "wrongpass")
        logger.info(f"Auth Test 2: {auth_success}, {auth_msg}")
        ...
        logger.info(f"Reg Test 1: {reg_success}, {reg_msg}")
